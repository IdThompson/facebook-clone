...
<head>
  ...
  <%
    String message=(String)request.getAttribute("emptypost");%>
</head>
<body>
<div class="header">
  ...
  <div class="header__right">
    <div class="header__info">
      <img class="user__avatar"
           ...
           alt=""/>
      <a id="my-name" href="<%=request.getContextPath()%>/dashboard.jsp">${user.userName}</a>
    </div>
...
<%--content body--%>
<div>
  <%--        <c:out value="${userPosts.get(0).content}"></c:out>--%>
  <%--    <c:if test="${usersPosts.size()!=0}">--%>
  <c:forEach var="userPost" items="${usersPosts}">
    <div class="post-contents" style="margin: 10%">
      <div class="post__top">
        <img
      ...
      <div class="post-attributes" style="display: flex;flex-direction: row;align-items: center;">
        <form action="<%=request.getContextPath()%>/deleted" method="post">
          <input type="hidden" name="id" value="${userPost.postId}"/>
          <button type="submit"class="btn btn-secondary btn-lg">delete</button>
        </form>
        <br><br>
        <form action="<%=request.getContextPath()%>/edit" method="post">
          <input type="hidden" name="id" value="${userPost.postId}"/>
            <%--        <input type="hidden" name="value=${userPost.content}"/>--%>
